...
Polinom::Monom Polinom::ParseMonom(const string& monomStr) {
	double coef = 0;
...
void Polinom::ParseFromString(const string& p) {
	vector<string> m;
	string cur_m;
...
Polinom::Polinom(const string& polStr) {
...
ostream& operator<<(ostream& os, const Polinom& polinom) {
	...
	for (size_t i = 0; i < polinom.monoms.size(); ++i) {
		const Polinom::Monom& monom = polinom.monoms[i];
		if (monom.coef != 0) {
			if (!isFirst) {
				os << (monom.coef > 0 ? " + " : " - ");
			}
...
Polinom operator*(double c, const Polinom& p) {
	return p * c;
}
